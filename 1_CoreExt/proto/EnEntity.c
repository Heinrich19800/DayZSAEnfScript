/**
 * \defgroup EntityAPI Entity API definition
 * @{
 */
...
enum EntityEvent
{
	//!dotkla se entita - other
	TOUCH,

	/*!
	je vybran pro rendering
	other		WorldClass
	extra		cislo snimku
	...
	*/
	...
	/*!
	kazdy frame, jeste pred fyzikou
	other		WorldClass
	...
	*/
	...
	/*!
	POSTFRAME - - ||- pote co se s ni pohne
	other		WorldClass
	*/
	POSTFRAME,

	/*!
	zavolana po nahrani celeho levelu
	other		WorldClass
	*/
	INIT,      	

//Jen u MeshObject:
//ANIMEND	- Konec AF_ONCE animace, extra = cislo slotu 0...11
//ANIMBLEND - Animace preblendovala z predchozi. extra = cislo slotu 0...11


//SOUNDEND	- skoncilo prehravani SFL_ONCE zvuku
...
*/
//@{
	private void EOnTouch(IEntity other, int extra) //!EntityEvent.TOUCH
	{
	}
	event private void EOnInit(IEntity other, int extra) //!EntityEvent.INIT
	{
	}
	private void EOnExtra(IEntity other, int extra)	//!EntityEvent.EXTRA
	{
	}
	private void EOnNotVisible(IEntity other, int extra) //!EntityEvent.NOTVISIBLE
	{
	}
	...
	private void EOnWorldProcess(IEntity other, int extra) //!EntityEvent.WORLDPROCESS
	{
	}
	private void EOnAnimEvent(IEntity other, AnimEvent extra) //!EntityEvent.ANIMEVENT
	{
	}
	private void EOnSoundEvent(IEntity other, SoundEvent extra) //!EntityEvent.SOUNDEVENT
	{
	}
	private void EOnSimulate(IEntity other, float dt) //!EntityEvent.SIMULATE
	{
	}
	private void EOnPostSimulate(IEntity other, int extra)	//!EntityEvent.POSTSIMULATE
	{
	}
	private void EOnJointBreak(IEntity other, int extra) //!EntityEvent.JOINTBREAK
	{
	}
	private void EOnPhysicsMove(IEntity other, int extra) //!EntityEvent.PHYSICSMOVE
	{
	}
	private void EOnContact(IEntity other, Contact extra) //!EntityEvent.CONTACT
	{
	}
...
//DO NOT INSERT ANYTHING ABOVE - order of event methods matters!
	...
	*/
	...
	\brief Returns transformation of Entity. It returns only so much vectors as array is big
		...
		@endcode
	*/
	proto void GetTransform(out vector mat[]);
	...
	\brief Returns one row of Entity transformation matrix
		...
		@endcode
	*/
	proto native vector GetTransformAxis(int axis);
	...
	*/
	...
	\brief Returns origin of Entity
		...
		@endcode
	*/
	proto native vector GetOrigin();
	
	/**
	\brief Returns orientation of Entity in world space
		...
		@endcode
	*/
	proto native vector GetAngles();

	/**
	\brief Returns local orientation when it's in hierarchy
		\return \p vector local orientation
		@code
			Man player = g_Game.GetPlayer();
			Print( player.GetLocalAngles() );
	
			>> <180,-57.2585,180>
		@endcode
	*/
	proto native vector GetLocalAngles();

	/**
	\brief Sets angles for entity
		\param angles \p vector angles to be set
		@code
			Man player = g_Game.GetPlayer();
			player.SetAngles("180 50 180" );
			Print( player.GetAngles() );
	
			>> <-180,50,-180>
		@endcode
	*/
	proto native void SetAngles(vector angles);
	...
	\brief Sets origin for entity
		...
		@endcode
	*/
	proto native void SetOrigin(vector orig);
	...
	*/
	...
	proto native int AddChild(notnull IEntity child, int pivot);
	//!Remove Entity from hierarchy
	proto native void RemoveChild(notnull IEntity child);
	...
	\brief Returns local bounding box of model on Entity
		...
		@endcode
	*/
	proto void GetBounds(out vector mins, out vector maxs);
	...
	\brief Returns quantized world-bound-box of Entity
		...
		@endcode
	*/
	proto void GetWorldBounds(out vector mins, out vector maxs);
	...
	*/
	...
	\brief Returns Entity flags
		...
		@endcode
	*/
	proto native EntityFlags GetFlags();
	...
	\brief Sets Entity flags. It's OR operation, not rewrite. Returns previous flags
		...
		@endcode
	*/
	proto native EntityFlags SetFlags(EntityFlags flags, bool recursively);
	...
	\brief Clear Entity flags. Returns cleared flags
		...
		@endcode
	*/
	proto native EntityFlags ClearFlags(EntityFlags flags, bool recursively);
	...
	\brief Returns current event mask
		...
		@endcode
	*/
	proto native EntityEvent GetEventMask();
	...
	\brief Sets event mask
		...
		@endcode
	*/
	proto native void SetEventMask(EntityEvent e );
	...
	\brief Clears event mask
		...
		@endcode
	*/
	proto native EntityEvent ClearEventMask(EntityEvent e);
	
	//!Dynamic event invokation. Parameters are the same as in IEntity::EOnXXXX() methods
	proto volatile void SendEvent(notnull IEntity actor, EntityEvent e, void extra);
...
//@{
	...
	*/
	...
	proto native int Animate(float speed, int loop);
	//!Updates animation (either xob, or particle, whatever)
	proto native int AnimateEx(float speed, int loop, out vector lin, out vector ang);

	//!Sets visibility mask for cameras, where Entity will be rendered
	proto native int SetCameraMask(int mask);	
//@}

	/**
	\brief When called, the Entity is excluded from consequent TraceMove/TraceLine
	*/
	proto native void FilterNextTrace();
...
class ParamEnumArray: array<ParamEnum>
{
	void ~ParamEnumArray()
	{
...
{
	string m_DefValue;
	string m_UiWidget; ///< can be "editbox", "combobox", "spinbox", "slider", "font", "fileeditbox", "color", "flags"
	string m_RangeScale; ///< defined as "MIN_VALUE MAX_VALUE STEP" eg. "1 100 0.5"
	string m_Desc;
	autoptr ParamEnumArray m_Enums; ///< Only ints and floats are currently supported. Array can be defined this way: { ParamEnum("Choice 1", "1"), ParamEnum("Choicen 2", "2") }
...
class EditorAttribute
{
	...
	void EditorAttribute(string style, string category, vector sizeMin, vector sizeMax, string color, string color2 = "0 0 0 0", bool visible = true, bool insertable = true, bool dynamicBox = false)
	{
		m_Style = style;
		m_Category = category;
		m_SizeMin = sizeMin;
